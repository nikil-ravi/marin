...
from marin.evaluation.evaluators.lm_eval_length_utils import (
    normalize_model_args_for_task,
    resolve_lm_eval_max_length,
)
...
def test_convert_to_levanter_task_config_preserves_metadata() -> None:
    pytest.importorskip("levanter.eval_harness")
...
def test_resolve_levanter_lm_eval_max_length_respects_task_lengths() -> None:
    ...
    assert resolve_lm_eval_max_length(None, evals) == 16384
    assert resolve_lm_eval_max_length({"max_length": 8192}, evals) == 16384
    assert resolve_lm_eval_max_length({"max_model_len": 32768}, evals) == 32768
...
def test_normalize_model_args_for_task_maps_max_model_len_and_task_requirement() -> None:
    ...
    normalized = normalize_model_args_for_task(args, no_metadata_task)
    assert _get_arg(normalized, "max_length") == "32768"

    long_task = EvalTaskConfig("ruler_vt", 0, task_kwargs={"max_seq_lengths": [4096, 65536]})
    normalized_long = normalize_model_args_for_task(args, long_task)
